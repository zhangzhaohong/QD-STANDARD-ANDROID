 ...
 * @描述 调色板网格适配器
 */
public class PaletteGridAdapter_Music_List extends RecyclerView.Adapter<PaletteGridAdapter_Music_List.PaletteGridViewHolder> {
    ...
    class PaletteGridViewHolder extends RecyclerView.ViewHolder {
        ...
        private void jump_to_Player() {
            if (url[getAdapterPosition()] == null){
                ...
                if (url[getAdapterPosition()].equals("")) {
                    ...
                    /*if (Music_player_manager.getHot_music_service() != null)
                        FToastUtils.init().setRoundRadius(30).show("地址不合规，无法播放！");
                    else if (Music_player_manager.getHot_music_service().equals(""))
                        FToastUtils.init().setRoundRadius(30).show("地址不合规，无法播放！");
                    else{
                        Music_player_manager.changeService("HotMusic");
                        LogUtil.d("Hot-" + Music_player_manager.getHot_music_service());
                    }*/
                    //Music_player_manager.setMusicService(Music_player_manager.getHot_music_service());
                    Intent intent = new Intent(mContext, MusicPlayerActivity.class);
                    Bundle bundle = new Bundle();
                    bundle.putInt("id", getAdapterPosition());
                    bundle.putString("from","MusicList");
        ...
        private void long_click_operation() {
            ...
            }else if (NetworkChecker.isWifiProxy(Music_list_activity.getAppContext())){
                if (tipDialog.isShowing())
                    tipDialog.dismiss();
        ...
        private void getInfo(int id) {
            ...
            if (Host_manager.getHotMusicHost() == null){
                ...
                switch (MUSIC_SERVICE) {
                    case "netease":
                        key = Host_manager.getHotMusicHost() + "/" + MUSIC_SERVICE + "/url?id=" + getURLEncoderString(url[id]) + "&quality=" + getURLEncoderString(Netease_quality);
                        break;
                    case "tencent":
                        key = Host_manager.getHotMusicHost() + "/" + MUSIC_SERVICE + "/url?id=" + getURLEncoderString(url[id]) + "&quality=" + getURLEncoderString(Tencent_quality);
                        break;
                    case "kugou":
                        key = Host_manager.getHotMusicHost() + "/" + MUSIC_SERVICE + "/url?id=" + getURLEncoderString(url[id]) + "&quality=" + getURLEncoderString(Kugou_quality);
                        break;
                    case "kuwo":
                        key = Host_manager.getHotMusicHost() + "/" + MUSIC_SERVICE + "/url?id=" + getURLEncoderString(url[id]) + "&quality=" + getURLEncoderString(Kuwo_quality);
                        break;
                    case "migu":
                        key = Host_manager.getHotMusicHost() + "/" + MUSIC_SERVICE + "/url?id=" + getURLEncoderString(url[id]) + "&quality=" + getURLEncoderString(Migu_quality);
                        break;
                    case "baidu":
                        key = Host_manager.getHotMusicHost() + "/" + MUSIC_SERVICE + "/url?id=" + getURLEncoderString(url[id]) + "&quality=" + getURLEncoderString(Baidu_quality);
                        break;
                }
            }
            init_service();
        ...
        private void showProgress(Boolean status){
            if (status){
                tipDialog = new QMUITipDialog.Builder(Music_list_activity.getAppContext())
                        ...
                        if (String.valueOf(msg.obj).equals("访问成功")){
                            ...
                            if (url_302 == null) {
                                ...
                                if (tipDialog.isShowing())
                                    tipDialog.dismiss();
                                try {
                                    downManger = new DownManager(Music_list_activity.getAppContext());
