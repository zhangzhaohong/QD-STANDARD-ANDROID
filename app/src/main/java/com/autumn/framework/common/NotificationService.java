package com.autumn.framework.common;
...
import com.autumn.framework.po.XGNotification;
...
public class NotificationService {
	...
	public void save(XGNotification notification) {
		...
		values.put("content_entertainment_1", notification.getContent());
	...
	public XGNotification find(Integer id) {
		...
		Cursor cursor = db
				.query("notification",
						new String[] { "id,msg_id,title,content_entertainment_1,activity,notificationActionType,update_time" },
		...
		try {
			if (cursor.moveToFirst()) {
				return new XGNotification(cursor.getInt(cursor
						...
						.getColumnIndex("content_entertainment_1")), cursor.getString(cursor
